import { strings } from "./emailTemplates";
...
test.each(locales.filter((locale) => locale !== "en"))(
  "emailTemplates strings exposes the same keys in English and %s",
  (locale) => {
    expect(keyShape(strings[locale])).toEqual(keyShape(strings.en));
  }
);

test.each(locales)("emailTemplates strings has non-empty %s copy", (locale) => {
