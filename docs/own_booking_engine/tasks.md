...
[x] 3.10 Calendar dot styling: implement `.price-dot` SCSS with color variants (green #4CAF50, yellow #FFC107, red #F44336, grey #BDBDBD); test at mobile breakpoints; ensure dots are visible without cluttering the calendar grid. (4h)  
...
[x] 7.3 Load + abuse testing: simulate bot traffic on search/hold endpoints; validate rate limits and cost controls. (12h)  
[x] 7.3a CAPTCHA provider integration: wire up inbound X-Captcha-Token verification against reCAPTCHA/hCaptcha in AbuseGuard (see TODO in abuseProtection.ts); add end-to-end test for the bypass path. (8h)  
[x] 7.4 Launch readiness: runbooks, alerts, on-call plan, rollback plan, post-launch monitoring checklist. (10h)
