# Secure, production-grade booking engine with Smoobu API, PayPal, and manual deposit flow

## Executive summary

Building a secure, production-grade booking engine on top of the Smoobu API with **two payment methods (PayPal + manual deposit)** is feasible, but only if you introduce a **backend-only booking API**, a **database-backed state machine**, and **webhook-driven reconciliation** (Smoobu + PayPal). Smoobu explicitly notes that, for security, its API should not be called from a front-end; you should place a backend proxy in between. citeturn25view2

The major security risks to manage are (a) **calendar/availability abuse** (bots “reserving all slots”), (b) **race conditions / double bookings** between “availability shown” and “booking created,” and (c) **payment confirmation integrity** (ensuring what you mark “paid” corresponds to a verified PayPal capture or an admin-verified deposit receipt). OWASP explicitly calls out “making a reservation” as a sensitive business flow that attackers can automate to deny service to legitimate users. citeturn23view0
...
- **Hold/booking**: for both PayPal and deposit flows, create a *real, expiring hold* in your DB and (recommended) a *real, expiring hold* in Smoobu (see Option table) using Smoobu’s “Blocked channel” (id `11`) or “Direct booking” (id `13`) depending on how you want it to appear in Smoobu. citeturn17search0turn16search0turn25view0  
...
- **Deposit**: collect details → create hold → show instructions + countdown → upload receipt via signed URL → admin review/approve → confirm booking; if the countdown expires, **auto-cancel** (recommended to be real, not fake). This improves data integrity and prevents indefinite “inventory lock.” (Also: you can still provide a “Need help?” button that extends the hold or flags it for manual exception.)
...
- Best practices for **file upload security** (receipt uploads), **secrets management**, **encryption**, **session/timeouts**, and **rate limiting**. citeturn8search7turn15search8turn15search0turn8search2turn15search4  
...
- That iFrame approach reduces backend complexity, but it **cannot** satisfy your custom manual-deposit workflow safely, because you need server-side holds, uploads, admin approval, and unforgeable booking/payment state.  
...
A safe model is to treat every guest attempt as a **booking session** that transitions states only via server-validated events (user actions, webhook events, admin actions).
...
### Deposit flow design with an explicit “hold decision”

You asked for a deposit workflow:
...
A production-grade system should make this **timer real** at the backend. This is not just UX: it protects inventory integrity and prevents indefinite locks.
...
#### Two options for deposit holds

| Decision | Option A: Provisional Smoobu reservation/blocked booking | Option B: Local-only hold (DB) + re-check later |
|---|---|---|
| What you do | Immediately create a Smoobu hold (blocked or direct booking) **and** store hold in DB. Cancel if unpaid by deadline. citeturn17search0turn16search0 | Store hold in your DB only; do **not** touch Smoobu until proof verified. |
| Double-booking risk | Lowest: Smoobu calendar is actually blocked, so cross-channel sync reduces collisions. (Smoobu is built to eliminate double bookings with centralized calendar sync.) citeturn17search6turn16search0 | Higher: another channel booking can arrive while you’re waiting for deposit verification; you may have to reject/undo later. |
| Operational cleanliness | Smoobu calendar contains holds that later cancel; needs labeling. | Smoobu calendar stays clean; fewer cancellations. |
| Fraud/abuse resistance | Stronger: “reserve all slots” attacks still possible, so you must rate-limit/captcha anyway. citeturn23view0turn15search4 | Also vulnerable to abuse; but attackers only occupy your DB, not Smoobu. |
| Guest experience | Clear: “We hold your dates until X.” | Risk: “Dates were available but now aren’t,” causing frustration. |
| Recommendation | **Recommended for your goals** (“when a date is shown as available it actually is”). Use real expiration + auto-cancel to prevent inventory hostage-taking. citeturn23view0 | Only recommended if you cannot create holds in Smoobu or if your business prefers not to create provisional entries. |

**Recommendation: Option A**, with a strict, backend-driven expiry and automated cancellation.

#### Detailed deposit flow (recommended)
...
3. Guest selects “Deposit” payment method even if no properties are available (your requirement):  
   - UI still renders results page; if no properties: show “No houses available for these dates” (no error).  
4. Backend creates a **booking_session** and (if a property is available and selected) creates a **hold**:
   - In Smoobu: create a reservation on Blocked channel (`11`) or Direct booking (`13`). citeturn17search0turn16search0  
   - In DB: save hold with `expires_at = now + 60 minutes`.
5. Backend returns `deposit_instructions_url` and a server-generated `upload_token`.
6. UI shows deposit instructions with countdown (client timer) **and** polls backend for real status (the server expiry is authoritative).
7. Guest uploads receipt image:
   - Backend issues a **pre-signed upload URL** (see file upload security section).
   - After upload completes, backend records `receipt_uploaded_at` and moves state to `DEPOSIT_UNDER_REVIEW`.
8. Admin reviews:
   - Approve: backend confirms booking, updates Smoobu booking fields (`deposit`, `depositStatus`, etc.), sends confirmation communications. Smoobu supports updating booking with deposit/payment fields. citeturn16search0  
   - Reject: backend requests new upload or cancels hold and cancels Smoobu reservation. citeturn16search0  
9. If expired and not in “help/contact” exception:
   - backend auto-cancels hold and cancels Smoobu reservation (`DELETE /api/reservations/<reservationId>`). citeturn16search0  
...
## Security, privacy, monitoring, and admin operations
...
For “upload a picture of the deposit,” treat uploads as adversarial:
...
- Enforce:
  ...
  - virus scanning (async) before making available to admins. OWASP’s File Upload Cheat Sheet explicitly recommends validating size/type and treating uploaded files as untrusted. citeturn8search7  
...
- Offer a *“Need help?”* button that:
  - extends expiry once (e.g., +24h) and flags it for admin review,
  - logs a security event (potential abuse pattern),
  - sends an email/SMS to your staff.
...
- Log security-relevant events: failed webhook verifications, repeated hold attempts, rate-limit triggers, upload validation failures, booking state inconsistencies.
...
- Admin queue: “Deposit receipts pending review”
- Actions: approve, reject (request re-upload), cancel hold, manually extend hold, mark as “contacted support”
- Audit trail: who approved/changed what and when
- Dispute flow: if PayPal webhook indicates reversal/uncaptured state, put booking into “needs attention” and notify staff. citeturn13search10turn22search5  
...
- **Deposit receipt uploaded but malicious**: quarantine until scan passes; show only to authorized admins. citeturn8search7  
...
**Goal**: Guests can search any future date range and either (a) book with PayPal (instant confirmation) or (b) book via deposit (hold + receipt upload + manual approval), while ensuring your inventory/availability is correct, secrets are protected, and the booking state is tamper-resistant.
...
- Partial payments beyond deposit receipt workflow
...
- Payment method selection: PayPal or Deposit
- PayPal: order create/approve/capture, confirm booking
- Deposit: hold + receipt upload + admin approval, confirm booking
...
- Admin UI: deposit review + booking management + audit logs

**Non-functional requirements**
- Security: backend-only keys, verified webhooks, rate limits, upload hardening
...
- `POST /api/deposit/receipt/presign`
  - returns pre-signed upload URL + upload token
- `POST /api/deposit/receipt/complete`
  - confirms upload; transitions to “under review”
...
- `payment`
  - booking_session_id
  - method (paypal|deposit)
  - paypal_order_id, paypal_capture_id, status
  - deposit_receipt_file_id, status
- `uploaded_file`
  - storage_key, mime, size, scan_status, created_at
- `webhook_event`
  - provider (paypal|smoobu)
  - external_event_id, received_at, processed_at, status, payload_hash
- `audit_log`
  - actor (admin id), action, object_type, object_id, before/after
...
- Deposit receipt upload:
  - reject disallowed MIME/size; quarantine unknowns citeturn8search7  
- Hold expiration:
...
| Payment method selected | `payment_method_selected` | `add_payment_info` | `AddPaymentInfo` | user selects PayPal/Deposit | payment_type, value, currency citeturn24search0turn24search2 |
...
| Deposit receipt uploaded | `deposit_receipt_uploaded` | custom | `Lead` (optional) | upload complete | file_id, value |
| Deposit approved | `deposit_approved` | custom | — | admin approval | admin_id |
...
gantt
  ...
  Deposit flow + upload + admin review: c2, after b3, 12d
...
4. Add deposit workflow with hardened uploads (signed URLs + scanning + allowlists). citeturn8search7turn8search11  
5. Only after the above is stable: implement the reservation portal, analytics event mapping, and admin UX enhancements.
...
[ ] 4.3 Deposit instructions page: show bank instructions + real countdown tied to backend expiry; polling for status. (8h)  
[ ] 4.4 Secure receipt upload: implement presigned upload URLs, MIME/size allowlist, quarantine + virus scan pipeline, admin-only access. (16h)  
[ ] 4.5 Admin review UI: queue, approve/reject, request re-upload, extend hold, audit trail. (16h)  
...
[ ] 6.2 Guest communication templates: implement email/SMS templates for each state (hold created, payment pending, confirmed, cancelled, receipt requested). (8h)  
[ ] 6.3 Reservation portal auth: reservation ID + password creation, hashing, rate limiting, session management. (14h)  
[ ] 6.4 Portal pages: reservation details, upload/re-upload receipt, request cancellation/help. (14h)  
