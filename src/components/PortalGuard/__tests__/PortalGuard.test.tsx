...
function renderGuard(path: string) {
  return render(
    <MemoryRouter initialEntries={[path]}>
      <Routes>
        <Route
          path="/en/portal/:reservationPublicId"
          element={
            <PortalGuard>
        ...
        />
        <Route
          path="/es/portal/:reservationPublicId"
          element={
            <PortalGuard>
        ...
        />
        <Route path="/en/portal" element={<LocationProbe />} />
        <Route path="/es/portal" element={<LocationProbe />} />
      </Routes>
    </MemoryRouter>
  );
}
...
test('renders the reservation when a portal session exists', () => {
  ...
  renderGuard('/en/portal/KWL-ABCD1234');
...
test('redirects to login with the reservation id when there is no session', () => {
  ...
  expect(screen.getByTestId('location')).toHaveTextContent('/en/portal?reservationId=KWL-ABCD1234');
});

test('redirects to the Spanish login for Spanish portal URLs', () => {
  renderGuard('/es/portal/KWL-ABCD1234');

  expect(screen.getByTestId('location')).toHaveTextContent('/es/portal?reservationId=KWL-ABCD1234');
});

test('encodes the reservation id in the redirect', () => {
  renderGuard('/en/portal/KWL%20ABCD%261234');
