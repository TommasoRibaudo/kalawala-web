...
const BocasDelToro = () => {
    ...
    const m = useMessages();
    const content = bocasDelToroContent(locale);
    ...
    return (
        <div className={`listingContainer`}>
            <Helmet>
                ...
                <link rel="canonical" href={canonicalUrl('blogBocas', locale)} />
                {hreflangLinks('blogBocas')}
            </Helmet>
            ...
            <Row className="subContainer" style={{ justifyContent: 'center' }}>
                <Col className="info col" lg={{ order: 'first', span: 8 }} md={{ order: 'first', span: 10 }} sm={12} xs={12}>
                    ...
                    <div className="description" style={{ maxWidth: 1000 }}>
                        ...
                        <h2>{content.diyHeading}</h2>
                        <p>{content.diyIntro}</p>
                        <br />
                        <div style={{ overflowX: 'auto', marginBottom: '20px' }}>
                            <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #ddd' }}>
                                <thead>
                                    <tr style={{ backgroundColor: '#f5f5f5' }}>
                                        <th style={{ ...cell, textAlign: 'left' }}>{content.diyColLeg}</th>
                                        <th style={{ ...cell, textAlign: 'left' }}>{content.diyColCost}</th>
                                        <th style={{ ...cell, textAlign: 'left' }}>{content.diyColTime}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {content.diyLegs.map((r, i) => (
                                        <tr key={r.leg} style={i % 2 === 1 ? { backgroundColor: '#f9f9f9' } : undefined}>
                                            <td style={cell}><strong>{r.leg}</strong></td>
                                            <td style={cell}>{r.cost}</td>
                                            <td style={cell}>{r.time}</td>
                                        </tr>
                                    ))}
                        ...
                        <p>
                            <Link to={pathForKey('blogBushours', locale)}><strong>{content.diyLinkText}</strong></Link>
                        </p>
                        <br />

                        <h2>{content.requirementsHeading}</h2>
                        {content.requirementsParagraphs.map((p, i) => <p key={i}>{p}</p>)}
                        <br />
                        ...
                        <div style={{ overflowX: 'auto', marginBottom: '20px' }}>
                            <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #ddd' }}>
                                <thead>
                                    <tr style={{ backgroundColor: '#f5f5f5' }}>
                                        <th style={{ ...cell, textAlign: 'left' }}>{content.shuttleColOperator}</th>
                                        <th style={{ ...cell, textAlign: 'left' }}>{content.shuttleColDeparts}</th>
                                        <th style={{ ...cell, textAlign: 'left' }}>{content.shuttleColPrice}</th>
                                        <th style={{ ...cell, textAlign: 'left' }}>{content.shuttleColDuration}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {content.shuttleOperators.map((r, i) => (
                                        <tr key={r.operator} style={i % 2 === 1 ? { backgroundColor: '#f9f9f9' } : undefined}>
                                            <td style={cell}><strong>{r.operator}</strong></td>
                                            <td style={cell}>{r.departs}</td>
                    ...
                    <OtherBlogs currentBlog="puerto-viejo-to-bocas-del-toro" locale={locale} />
                </Col>
            </Row>
