...
const ListingRana = () => {
    ...
    const m = useMessages();
    const content = listingContent('Rana', locale);
    const listing = `Rana${localeSuffix(locale)}`
    ...
    return (
        <div className={`listingContainer${show ? ' modal-open' : ''}`}>
            <Helmet>
                ...
                <link rel="canonical" href={canonicalUrl('rana', locale)} />
                {hreflangLinks('rana')}
                <script type="application/ld+json">{JSON.stringify(vacationRentalJsonLd('Rana', locale, houseData?.image))}</script>
            </Helmet>
            ...
            <Row className="subContainer">
                <Col className="info col" lg={{ order: 'first', span: 10 }} md={{ order: 'first', span: 12 }} sm={12} xs={12}>
                    <div className="heading">
                        ...
                        <ListingMarketingSection propertyKey="Rana" locale={locale} />
                    </div>
                    <ImagesContainer showModal={handleShow} houseName={listing!} />
                    {/* Add social statement after images */}
                    <SocialStatement propertyKey="Rana" locale={locale} />
                    <div className="amenaties">
                        <Amenities amenities={houseData?.amenities as AmenityType[]} propertyKey="Rana" locale={locale} />
                    </div>

                    {/* Add feature highlights before description */}
                    <FeatureHighlights propertyKey="Rana" propertyName={content.featureName} locale={locale} />
                ...
                <Col id="smoobuComp" className="book col" lg={2} md={{ span: 12 }} sm={{ span: 12 }} xs={{ span: 12 }}>
                    <PriceConfirmationSection propertyKey="Rana" locale={locale} />
                    <BookingSearchWidget ref={widgetRef} locale={locale} defaultGuests={houseData!.guestNumber} variant="sidebar" apartmentSlug="Rana" />
                </Col>
            </Row>
