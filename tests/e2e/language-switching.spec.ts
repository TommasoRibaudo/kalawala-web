...
test.describe('Language Switching', () => {
  test('switching language on "/" navigates to "/HomeES"', async ({ appPage }) => {
    ...
    await appPage.waitForURL('**/HomeES');
    expect(appPage.url()).toContain('/HomeES');
  });
  ...
  test('switching language on a Spanish page navigates to the English equivalent', async ({ appPage }) => {
    ...
    // /HomeES → / (the root English homepage)
    await appPage.waitForURL(/\/$/);
    expect(appPage.url()).toMatch(/localhost:\d+\/$/);
  });

  test('switching language on "/Geco" navigates to "/GecoES"', async ({ appPage }) => {
    ...
    await appPage.waitForURL('**/GecoES');
    expect(appPage.url()).toContain('/GecoES');
  });
});
